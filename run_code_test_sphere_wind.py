...
from afterglow_lineofsight_bmk_morsony_mem_reduced_small_wind import *


# Where to save energy distibution
savefile = './savefile.short.onaxis.sphere.wind.p'   #.n-4
...
outputfile = './afterglow_sych_spectrum.sphere.wind.p'   # .n-4
...
epsilon = 0.1        #electron energy fraction
eqparb = 0.1        #magnetic energy fraction
pindex = 2.5 #2.01 #2.5 #2.335643   # electron spectrum index
...
#external_nodensity = 1.e-2 #1.0e0 #1.e-3 #1.0    ## for ISM


# For wind, use mass loss rate and wind velocity to set dentity at 1 cm (parameter A in Blandford & McKee)
#wind_mass_loss_rate = 1.e-5*2.e33 / (365.25*24.*3600) # (grams/s)
#wind_velocity = 1000.*1000.*100. # (cm/s) 
#    
#external_nodensity = wind_mass_loss_rate/wind_velocity / (4.*np.pi) / 1.67e-24

v_wind = 1000.      # km/s
m_dot_wind = 1.e-7 # 1e-5 #1.e-5  #*24.  ;3.d-5  ; m_sun/year
external_nodensity = (m_dot_wind*1.98892*1.e33/(365.25*24.*3600.))/(v_wind*1.e5)/4./np.pi/1.67e-24 #/ 1.67e-24  # for wind ;We're in a wind situation --Dominic, 11/30/13


print('external_nodensity = ',external_nodensity)
    
    # k_index is 0 for ISM external density, 2 for wind external density   
k_index = 2. # for wind  # 0. #for ISM
...
phi_slices = 2 #54 #18 #54  #18  # 18 is alright, 54 is probably better.  If you are doing an on-axis models, you can set this to 2.
    # Number of points in vertical direction to use for each slice (y points)
s_points = 30 #30 #15 #30    # 30 seems okay
    # Number of points into the shell to use for each slice (x points)
z_points = 100 #100 #50 #100   # 100 seems okay
...
opening_angle = 90. #90. #16. #.05*180/np.pi #10. #16. # 10.
...
eiso = 1.e52 #* 1.e28 #1e53 #3.1e49 #1.5e49 #1e47 #2.5e51 #1.7e51 # 1.e49  # 2.5e51 # 1.e51
...
gamma_init = 400. #'file' #4. #400 #5.0 #1.7 #200.

    # shock_thikcness is the thickness of the shock, in light seconds
shock_thickness = 0.1 #1.e3 #0.1  # * gv.cl 
